...
namespace ControlDeFinanzas.Servicios
{
    public class UsuarioStore : IUserStore<Usuario>, IUserEmailStore<Usuario>, IUserPasswordStore<Usuario>
    {
        ...
        {
            return await repositorioUsuarios.BuscarUsuarioPorEamil(normalizedEmail);
        }
        ...
        {
            return await repositorioUsuarios.BuscarUsuarioPorEamil(normalizedUserName);
        }

        public Task<string> GetEmailAsync(Usuario user, CancellationToken cancellationToken)
        {
            return Task.FromResult(user.Emial);
        }
